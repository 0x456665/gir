use crate::init::utils::{GitRepository, build_dir, build_file, repo_default_config};
...
pub fn init_repo(path: PathBuf) -> Result<GitRepository, String> {
    ...
    fs::write(
        build_file(&repo, &["description"], false).ok_or("Failed to build description file path")?,
